...
/* CMSIS-free barrier shims (standalone build has no core_cm4.h) */
#include <stdint.h>
static inline void __schedule_barrier_impl(void) { __asm volatile ("" ::: "memory"); }
#define __schedule_barrier __schedule_barrier_impl
static inline void __isb_impl(unsigned v) { (void)v; __asm volatile ("isb" ::: "memory"); }
static inline void __dsb_impl(unsigned v) { (void)v; __asm volatile ("dsb" ::: "memory"); }
#define __isb(v) __isb_impl(v)
#define __dsb(v) __dsb_impl(v)

/* Whole-file C linkage: this TU is built by g++ but must export plain C
 * symbols to usbd_core / usb.cpp. */
#ifdef __cplusplus
extern "C" {
#endif

/* Adapter: older-API endpoint config struct used by this port. */
struct usbd_endpoint_cfg { uint8_t ep_addr; uint8_t ep_mps; uint8_t ep_type; };
 ...
 * @retval           >=0 success otherwise failure
 */
int usbd_ep_open_shim(const struct usbd_endpoint_cfg *ep_cfg);

int usbd_ep_open(uint8_t busid, const struct usb_endpoint_descriptor *ep)
{
  (void)busid;
  struct usbd_endpoint_cfg cfg;
  cfg.ep_addr = ep->bEndpointAddress;
  cfg.ep_mps  = USB_GET_MAXPACKETSIZE(ep->wMaxPacketSize);
  cfg.ep_type = USB_GET_ENDPOINT_TYPE(ep->bmAttributes);
  return usbd_ep_open_shim(&cfg);
}
